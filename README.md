...
-------------
# Moreh AI Framework validated-steps to reproduce
## Install (clone or use code in current path of ml-workbench repo): https://github.com/facebookresearch/dlrm
With MAF conda env activated (pytorch, python, moreh)
```bash
cd ml-workbench/pytorch-maf/dlrm
...
```
- Test with Caffe2 and compare with Torch, may different if Caffe2 not compiled with CPU features.
```bash
./test/dlrm_s_test.sh --use-gpu
```
## Edit some code if raise value issue:
Line 1793 of 'ml-workbench/pytorch-maf/dlrm/dlrm_s_pytorch.py': use `param.cpu().detach().numpy()` instead of `param.detach().cpu().numpy()`. However, may get this warning, but the model is run successfully on GPU: `ResourceWarning: Implicitly cleaning up <TemporaryDirectory '/tmp/tmp2lfhu8sf'>`
